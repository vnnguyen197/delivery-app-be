...
const WardPrisma = prisma.ward;

const seedWard = async () => {
  const cover = async (code, name, nameEn, fullName, fullNameEn, codeName, districtCode, unitId) => {
    const district = await DistrictPrisma.findUnique({ where: { code: districtCode } });
    await WardPrisma.create({
      data: {
        code,
        name,
        nameEn,
